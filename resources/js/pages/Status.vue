    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="min-h-screen bg-neutral-50 dark:bg-transparent">
            <div class="max-w-full mx-auto px-2 sm:px-6 lg:px-12 py-8">
                <div class="mb-8">
                    <h1 class="text-4xl font-bold text-neutral-900 dark:text-white">Systémový Status & Ovládací Panel
                    </h1>
                ...
                <div class="mb-12">
                    <div
                        class="bg-white dark:bg-neutral-900 rounded-2xl shadow-lg border border-neutral-200 dark:border-neutral-700">
                        <div
                            class="px-8 py-6 border-b border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
                            <h2 class="text-2xl font-semibold text-neutral-900 dark:text-white">Status Serverů / Node
                                Ovládání</h2>
                            <button @click="refreshNodes" :disabled="loadingNodes"
                                class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-base rounded-lg transition-all duration-200 shadow">Obnovit</button>
                        </div>
                        <div class="overflow-x-auto">
                            ...
                            <table v-else
                                class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700 text-base">
                                <thead class="bg-neutral-50 dark:bg-neutral-900">
                                    <tr>
                                        <th
                                            class="px-6 py-4 text-left text-xs font-bold text-neutral-500 dark:text-neutral-300 uppercase tracking-wider">
                                            Server</th>
                                        <th
                                            class="px-6 py-4 text-left text-xs font-bold text-neutral-500 dark:text-neutral-300 uppercase tracking-wider">
                                            Status</th>
                                ...
                                <tbody
                                    ...
                                    <tr v-for="node in nodes" :key="node.id"
                                        ...
                                        <td
                                            class="px-6 py-4 whitespace-nowrap font-semibold text-neutral-900 dark:text-white">
                                            {{ node.name }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap flex gap-2">
                                            <button @click="openConfirmModal(node, 'zapnout')"
                                                :disabled="nodeActionLoading[node.name]"
                                                class="px-3 py-1 bg-green-600 hover:bg-green-700 text-white text-xs rounded-lg shadow">Start</button>
                                            <button @click="openConfirmModal(node, 'restartovat')"
                                                :disabled="nodeActionLoading[node.name]"
                                                class="px-3 py-1 bg-orange-600 hover:bg-orange-700 text-white text-xs rounded-lg shadow">Restart</button>
                                            <button @click="openConfirmModal(node, 'vypnout')"
                                                :disabled="nodeActionLoading[node.name]"
                                                class="px-3 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded-lg shadow">Stop</button>
                                            <button @click="pingNode(node)" :disabled="nodeActionLoading[node.name]"
                                                class="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white text-xs rounded-lg shadow">Ping</button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div v-if="showConfirmModal"
                    ...
                    <div
                        class="bg-white dark:bg-neutral-900 rounded-xl shadow-2xl p-8 max-w-sm w-full text-center animate-in zoom-in-95 duration-200">
                        ...
                        <p class="mb-6 text-neutral-700 dark:text-neutral-300">
                            ...
                            }">{{ confirmActionTypeText }}</span> server <span class="font-bold">{{ confirmNode?.name
                                }}</span>?
                        </p>
